...
import { DTOLogin, DTORegister, DTOUsers } from '@dto/dto.users'
...
export class ServiceUsers extends Model {
  ...
  async allUsersService(req: Request<DTOUsers>): Promise<any> {
    try{
      const checkUsers: ModelUsers = await super.model().users.findOne({ email: req.body.email}, { relations: ['role'] })
      if(!checkUsers) throw apiResponse(status.NOT_FOUND, 'Users not found')
      return Promise.resolve(apiResponse(status.OK, 'Users'))
    }catch(e: any){
      return Promise.reject(apiResponse(e.stat_code || status.NOT_FOUND, e.stat_message || e.stat_message))
    }
  }
}
